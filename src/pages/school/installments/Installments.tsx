...
import { reserveReceiptNumbers, reserveBatchReceiptNumbers } from '../../../utils/receiptCounter';
...
// Payment info tooltip component for installments
const InstallmentPaymentInfoTooltip = ({ installment }: { installment: Installment }) => {
  // Only show for paid or partial installments
  if (installment.status !== 'paid' && installment.status !== 'partial') {
    return null;
  }
  
  const hasTrackingData = installment.paidBy || installment.paymentRecordedAt;
  
  return (
    <div className="relative group inline-block mr-2">
      <Info size={14} className={`cursor-help ${hasTrackingData ? 'text-blue-500' : 'text-gray-400'}`} />
      <div className="absolute z-50 hidden group-hover:block bg-white border border-gray-200 rounded-lg shadow-lg p-3 min-w-[220px] right-0 top-6">
        <div className="text-xs text-gray-600 space-y-1">
          <div className="font-semibold text-gray-800 border-b pb-1 mb-1">تفاصيل الدفع</div>
          {hasTrackingData ? (
            <>
              {installment.paidBy && (
                <div className="flex items-center gap-1">
                  <User size={12} />
                  <span>سجل بواسطة: {installment.paidBy}</span>
                </div>
              )}
              ...
              {installment.paidByEmail && (
                <div className="text-gray-500 text-[10px]">{installment.paidByEmail}</div>
              )}
              {installment.paymentRecordedAt && (
                <div className="flex items-center gap-1 text-gray-500">
                  <Clock size={12} />
                  <span>{new Date(installment.paymentRecordedAt).toLocaleString('ar-SA')}</span>
                </div>
              )}
            </>
          ) : (
            <div className="text-gray-400 italic">لا توجد بيانات تتبع (دفعة قديمة)</div>
          )}
        </div>
      </div>
    </div>
...
const Installments = () => {
    ...
    try {
      ...
      if (totalPaidForThisInstallment >= originalAmount) {
        ...
        await hybridApi.saveInstallment({
          ...
          // Payment tracking - who recorded this payment
          paidBy: user?.name || user?.email || 'Unknown',
          paidByRole: user?.role || 'unknown',
  ...
  const handleBulkInstallmentZipDownload = async () => {
    ...
    console.log(`[BulkZIP] Starting optimized bulk download for ${selectedIds.length} installment receipts...`);
    
    // ENABLE BULK MODE - prevents cache refresh during generation
    startBulkOperation();
    ...
    try {
      ...
      const selectedInstallmentsData = selectedIds
        .map(id => installments.find(i => i.id === id))
        .filter(Boolean) as Installment[];
      
      // OPTIMIZATION 1: Batch reserve ALL receipt numbers at once (single DB call)
      // Count installments that need new receipt numbers (unpaid ones being paid)
      const installmentsNeedingReceipts = selectedInstallmentsData.filter(inst => {
        // Only need receipt number if it doesn't have one already
        return !inst.receiptNumber || inst.receiptNumber.trim() === '';
      });
      
      let batchReceiptNumbers: string[] = [];
      if (installmentsNeedingReceipts.length > 0) {
        console.log(`[BulkZIP] Batch reserving ${installmentsNeedingReceipts.length} receipt numbers...`);
        batchReceiptNumbers = await reserveBatchReceiptNumbers(user?.schoolId || '', 'installment', installmentsNeedingReceipts.length);
        console.log(`[BulkZIP] Reserved ${batchReceiptNumbers.length} receipt numbers in single call`);
      }
      
      // OPTIMIZATION 2: Build students map for quick lookup
      console.log('[BulkZIP] Fetching settings and student data...');
      const studentsMap = new Map<string, any>();
      for (const student of students) {
        studentsMap.set(student.id, student);
      }
      
      // Pre-assign receipt numbers to installments that need them
      let receiptIndex = 0;
      const installmentsWithReceiptNumbers = selectedInstallmentsData.map(inst => {
        let receiptNumber = inst.receiptNumber || '';
        
        if (!receiptNumber && receiptIndex < batchReceiptNumbers.length) {
          receiptNumber = batchReceiptNumbers[receiptIndex++];
        }
        return { inst, receiptNumber };
      });
      
      // OPTIMIZATION 3: Generate PDFs in controlled batches
      console.log('[BulkZIP] Generating PDFs in controlled batches...');
      ...
      const generateSinglePDF = async (
        ...
        if (!student) {
          console.warn(`[BulkZIP] Student not found for installment ${inst.id}`);
          return null;
        }
        
        // Build receipt data directly
        const studentIdDisplay = inst.studentCustomId || student.studentCustomId || '';
        const isPartial = inst.status === 'partial';
        const instPaidAmount = inst.paidAmount || (inst.status === 'paid' ? inst.amount : 0);
        const paidAmount = isPartial ? instPaidAmount : (inst.status === 'paid' ? (instPaidAmount || inst.amount) : 0);
        const remainingAmount = Math.max(0, (inst.amount || 0) - paidAmount);
        
        // Use ISO date format to avoid parsing issues
        ...
        const receiptData = {
          ...
          englishName: inst.englishName || student.englishName || '',
          studentId: studentIdDisplay,
          studentInternalId: inst.studentId,
          grade: inst.grade,
          englishGrade: inst.englishGrade || student.englishGrade || '',
          feeType: getFeeTypeLabel(inst.feeType),
          amount: paidAmount,
        ...
        const html = await generateReceiptHTML(receiptData as any);
        ...
        const fileName = `Installment_${rnSafe}_${safeName}.pdf`;
        ...
        if ((window as any).electronAPI?.printToPdfWithPath) {
          ...
          const raw = fb?.data;
          if (raw instanceof Uint8Array) pdfData = raw;
          else if (Array.isArray(raw)) pdfData = new Uint8Array(raw);
          else if (raw?.data && Array.isArray(raw.data)) pdfData = new Uint8Array(raw.data);
        ...
        const totalReceiptTime = Date.now() - receiptStartTime;
        console.log(`[BulkZIP] Receipt ${globalIndex + 1}/${totalCount}: HTML=${htmlTime}ms, PDF=${pdfTime}ms, Total=${totalReceiptTime}ms`);
        
        if (pdfData) {
          return { name: fileName, data: pdfData };
        }
        return null;
      };
      
      // Generate PDFs in controlled batches to prevent browser overload
      const BATCH_SIZE = 2; // Generate 2 PDFs at a time for optimal performance
      const allResults: ({ name: string; data: Uint8Array } | null)[] = [];
      const totalCount = installmentsWithReceiptNumbers.length;
      
      for (let batchStart = 0; batchStart < totalCount; batchStart += BATCH_SIZE) {
        const batchEnd = Math.min(batchStart + BATCH_SIZE, totalCount);
        const batchItems = installmentsWithReceiptNumbers.slice(batchStart, batchEnd);
        const batchNum = Math.floor(batchStart / BATCH_SIZE) + 1;
        const totalBatches = Math.ceil(totalCount / BATCH_SIZE);
        
        console.log(`[BulkZIP] Generating batch ${batchNum}/${totalBatches} (${batchItems.length} PDFs)...`);
        const batchStartTime = Date.now();
        
        // Generate this batch in parallel
        const batchPromises = batchItems.map(({ inst, receiptNumber }, i) =>
          generateSinglePDF(inst, receiptNumber, batchStart + i, totalCount)
        );
        ...
        console.log(`[BulkZIP] Batch ${batchNum} completed in ${batchTime}ms`);
      }
      
      // Filter valid results from batched generation
      const filesForZip = allResults.filter((r): r is { name: string; data: Uint8Array } => r !== null);
      
      const parallelTime = Date.now() - parallelStartTime;
      console.log(`[BulkZIP] Generated ${filesForZip.length} PDFs in controlled batches in ${parallelTime}ms`);
      ...
      console.log(`[BulkZIP] Completed ${filesForZip.length} installment receipts in ${totalTime}ms (${Math.round(totalTime/filesForZip.length)}ms per receipt)`);
      ...
      // ALWAYS end bulk mode, even on error
      endBulkOperation();
      setBulkDownloadLoading(false);
    }
  };
                              ...
                              <tr key={installment.id} className="odd:bg-white even:bg-gray-50 hover:bg-gray-100">
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <div className="flex items-center gap-1">
                                    ...
                                    <InstallmentPaymentInfoTooltip installment={installment} />
                                  </div>
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap">
