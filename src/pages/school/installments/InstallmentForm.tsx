...
import { CURRENCY, INSTALLMENT_INTERVALS } from '../../../utils/constants';
...
const InstallmentForm = () => {
  ...
  const [formData, setFormData] = useState<InstallmentFormData>({
    ...
    installmentCount: 1,
    installmentInterval: 1, // Default to monthly
        ...
        if (isEditMode && id) {
          ...
          if (installment) {
            setFormData({
              ...
              installmentInterval: (installment as any).installmentInterval || 1, // Default to monthly if not set
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      for (let i = 0; i < formData.installmentCount; i++) {
        ...
        // Use installment interval to calculate date (multiply index by interval)
        installmentDate.setMonth(baseDate.getMonth() + (i * formData.installmentInterval));
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, installmentInterval: parseInt(e.target.value) }))}
                    className="w-full border rounded-md p-2"
                    disabled={isEditMode}
                  >
                    {INSTALLMENT_INTERVALS.map((interval) => (
