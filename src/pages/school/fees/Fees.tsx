...
// Payment info tooltip component
const PaymentInfoTooltip = ({ fee }: { fee: Fee }) => {
  // ALWAYS show for debugging - remove status check temporarily
  const hasTrackingData = fee.paidBy || fee.paymentRecordedAt;
            ...
            <>
              {fee.paidBy && (
                <div className="flex items-center gap-1">
                  <User size={12} />
                  <span>سجل بواسطة: {fee.paidBy}</span>
                </div>
              )}
              {fee.paidByRole && (
              ...
              {fee.paidByEmail && (
                <div className="text-gray-500 text-[10px]">{fee.paidByEmail}</div>
              )}
              {fee.paymentRecordedAt && (
                <div className="flex items-center gap-1 text-gray-500">
                  <Clock size={12} />
                  <span>{new Date(fee.paymentRecordedAt).toLocaleString('ar-SA')}</span>
                </div>
              )}
            </>
          ) : (
      ...
      <div className="bg-gradient-to-br from-blue-50/80 via-slate-50/60 to-gray-50/40 rounded-lg shadow-sm p-6 border border-gray-100">
        <div>
          ...
          <div className="flex flex-wrap gap-3 justify-start">
            ...
            {/* Bulk zip download button - OPTIMIZED for speed */}
            <button
              onClick={async () => {
                if (selectedFees.size === 0 || bulkDownloadLoading) return;
                setBulkDownloadLoading(true);
                const startTime = Date.now();
                console.log(`[BulkZIP] Starting optimized bulk download for ${selectedFees.size} receipts...`);
                
                // ENABLE BULK MODE - prevents cache refresh during generation
                hybridApi.startBulkOperation();
                ...
                try {
                  ...
                  // Count fees that need new receipt numbers
                  const feesNeedingReceipts = selectedFeesData.filter(fee => {
                    const extractRN = (note?: string) => {
                      if (!note) return '';
                      const m = note.match(/\[RN:([^\]]+)\]/);
                      return m ? m[1] : '';
                    };
                    const existingRN = (fee.receiptNumber && fee.receiptNumber.trim() !== '') 
                      ? fee.receiptNumber 
                      : extractRN(fee.paymentNote);
                    return !existingRN;
                  });
                  
                  let batchReceiptNumbers: string[] = [];
                  if (feesNeedingReceipts.length > 0) {
                    console.log(`[BulkZIP] Batch reserving ${feesNeedingReceipts.length} receipt numbers...`);
                    batchReceiptNumbers = await reserveBatchReceiptNumbers(user?.schoolId || '', 'fee', feesNeedingReceipts.length);
                    console.log(`[BulkZIP] Reserved ${batchReceiptNumbers.length} receipt numbers in single call`);
                  }
                  
                  // OPTIMIZATION 2: Fetch all required data upfront (batch queries)
                  console.log('[BulkZIP] Fetching settings and student data...');
                  const settingsResponse = await hybridApi.getSettings(user?.schoolId || '');
                  const schoolSettings = (settingsResponse?.success && settingsResponse?.data && settingsResponse.data.length > 0) 
                    ? settingsResponse.data[0] 
                    : null;
                  
                  // Get unique student IDs and fetch all students at once
                  const studentIds = [...new Set(selectedFeesData.map(f => f.studentId))];
                  const studentsMap = new Map<string, any>();
                  for (const studentId of studentIds) {
                    const studentResponse = await hybridApi.getStudent(studentId);
                    if (studentResponse?.success && studentResponse?.data) {
                      studentsMap.set(studentId, studentResponse.data);
                    }
                  }
                  
                  // OPTIMIZATION 3: Generate PDFs in PARALLEL without database saves
                  const tmpDir = 'C:\\Windows\\Temp';
                  
                  // Pre-assign receipt numbers to fees that need them
                  let receiptIndex = 0;
                  const extractRN = (note?: string) => {
                    if (!note) return '';
                    const m = note.match(/\[RN:([^\]]+)\]/);
                  ...
                  const feesWithReceiptNumbers = selectedFeesData.map(fee => {
                    let receiptNumber = (fee.receiptNumber && fee.receiptNumber.trim() !== '') 
                      ? fee.receiptNumber 
                      : extractRN(fee.paymentNote);
                  ...
                  const generateSinglePDF = async (
                    ...
                    if (!student) {
                      console.warn(`[BulkZIP] Student not found for fee ${fee.id}`);
                      return null;
                    }
                    
                    // Build receipt data directly (skip generateReceiptDataForFee to avoid DB calls)
                    const paidAmount = fee.paid || 0;
                    const totalFeeAmount = fee.amount || 0;
                    const discountAmount = fee.discount || 0;
                    const balanceAmount = fee.balance ?? (totalFeeAmount - discountAmount - paidAmount);
                  ...
                  for (let batchStart = 0; batchStart < totalCount; batchStart += BATCH_SIZE) {
                    ...
                    const batchPromises = batchItems.map(({ fee, receiptNumber }, i) => 
                      generateSinglePDF(fee, receiptNumber, batchStart + i, totalCount)
                    );
                  ...
                  console.log(`[BulkZIP] Completed ${filesForZip.length} receipts in ${totalTime}ms (${Math.round(totalTime/filesForZip.length)}ms per receipt)`);
                            ...
                            <table className="min-w-full divide-y divide-gray-200 border rounded-lg text-xs">
                              ...
                              <tbody className="bg-white divide-y divide-gray-200">
                                {student.transportationFees.map(fee => (
                                  <tr key={fee.id} className="hover:bg-gray-50" data-fee-section="transportation">
                                    <td className="px-4 py-4 whitespace-nowrap text-center">
                                      <input
                                        type="checkbox"
                                        className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
