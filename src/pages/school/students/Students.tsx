...
const Students = () => {
  ...
  const handleImportSuccess = (result: { studentsCount: number, feesCount: number, installmentsCount?: number }) => {
    ...
    // CRITICAL: Refresh students data immediately after import
    // Add small delay to ensure cache invalidation has completed
    console.log('[Students] Import completed successfully:', result);
    console.log('[Students] Triggering data refresh...');
    
    // Use setTimeout to ensure cache invalidation has propagated
    setTimeout(() => {
      console.log('[Students] Fetching fresh data after import...');
      fetchStudentsData();
    }, 100);
  };
