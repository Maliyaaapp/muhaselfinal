                    ...
                    {(() => { const sorted = filteredBySearch.slice().reverse(); const totalPages = Math.max(1, Math.ceil(sorted.length / pageSize)); const clampedPage = Math.min(currentPage, totalPages); const start = (clampedPage - 1) * pageSize; return sorted.slice(start, start + pageSize); })().map((message, idx) => (
                      <React.Fragment key={message.id || String(idx)}>
                        ...
                        {expandedMessageId === message.id && (
                          <tr className="bg-gray-50"><td colSpan={7} className="px-6 py-4"><div className="grid grid-cols-1 md:grid-cols-3 gap-4"><div className="bg-white p-4 rounded-xl border border-gray-200"><p className="text-xs text-gray-500 mb-1">القالب</p><p className="font-medium text-gray-800">{message.template || '—'}</p></div><div className="bg-white p-4 rounded-xl border border-gray-200"><p className="text-xs text-gray-500 mb-1">المصدر</p><p className="font-medium text-gray-800">{getSourceLabel(message)}</p></div><div className="bg-white p-4 rounded-xl border border-gray-200"><p className="text-xs text-gray-500 mb-1">الدور</p><p className="font-medium text-gray-800">{(() => { const role = (message as any).sentByRole || (message as any).sent_by_role; if (!role) return 'غير محدد'; if (role === 'schoolAdmin') return 'مدير المدرسة'; if (role === 'gradeManager') return 'مدير صف'; return role; })()}</p></div><div className="md:col-span-3 bg-white p-4 rounded-xl border border-gray-200"><p className="text-xs text-gray-500 mb-2">نص الرسالة</p><p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{message.message}</p></div></div></td></tr>
                        )}
                      </React.Fragment>
                    ))}
                  </tbody>
