...
// ============================================
// BULK MODE - Prevents cache operations during bulk operations
// ============================================
let bulkModeActive = false;

/**
 * Start bulk operation mode - disables cache refresh during bulk operations
 * Call this before starting bulk receipt generation, imports, etc.
 */
export const startBulkOperation = (): void => {
  bulkModeActive = true;
  console.log('🚀 [BulkMode] Started bulk operation - cache refresh disabled');
};

/**
 * End bulk operation mode - re-enables cache refresh
 * Call this after bulk operation completes
 */
export const endBulkOperation = (): void => {
  bulkModeActive = false;
  console.log('✅ [BulkMode] Ended bulk operation - cache refresh enabled');
};

/**
 * Check if bulk mode is active
 */
export const isBulkModeActive = (): boolean => {
...
export const getAll = async (table: string, filters?: Record<string, any>): Promise<ApiResponse> => {
  try {
    ...
    if (!lastRefreshTime[bgRefreshKey] || now - lastRefreshTime[bgRefreshKey] > 3000) {
      ...
      (async () => {
        // BULK MODE CHECK: Skip background refresh during bulk operations
        if (bulkModeActive) {
          console.log(`⏭️ [BulkMode] Skipping background cache refresh for ${table}`);
          return;
        }
  ...
  if (response.success && response.data) {
    ...
    const feesWithMappedFields = response.data.map((fee: any) => {
        ...
        return {
          ...
          paidBy: fee.paid_by,
          paidByRole: fee.paid_by_role,
          paidByEmail: fee.paid_by_email,
          paidById: fee.paid_by_id,
          paymentRecordedAt: fee.payment_recorded_at,
          paid_by: fee.paid_by,
          paid_by_role: fee.paid_by_role,
          paid_by_email: fee.paid_by_email,
          paid_by_id: fee.paid_by_id,
          payment_recorded_at: fee.payment_recorded_at
        };
      });
...
export const updateFee = async (id: string, feeData: any): Promise<ApiResponse> => {
  ...
  // Payment tracking fields - only include if they have values
  // This prevents errors if the columns don't exist in the database yet
  if (feeData.paidBy || feeData.paid_by) {
    mappedData.paid_by = feeData.paidBy || feeData.paid_by;
  }
  if (feeData.paidByRole || feeData.paid_by_role) {
    mappedData.paid_by_role = feeData.paidByRole || feeData.paid_by_role;
  }
  if (feeData.paidByEmail || feeData.paid_by_email) {
    mappedData.paid_by_email = feeData.paidByEmail || feeData.paid_by_email;
  }
  if (feeData.paidById || feeData.paid_by_id) {
    mappedData.paid_by_id = feeData.paidById || feeData.paid_by_id;
  }
  if (feeData.paymentRecordedAt || feeData.payment_recorded_at) {
    mappedData.payment_recorded_at = feeData.paymentRecordedAt || feeData.payment_recorded_at;
  }
  ...
  // Try to update with payment tracking fields first
  try {
    return await update(STORAGE_KEYS.FEES, id, mappedData);
  } catch (error: any) {
    // If we get a PGRST204 error about missing columns, retry without payment tracking fields
    if (error?.code === 'PGRST204' || error?.message?.includes('paidBy') || error?.message?.includes('paid_by')) {
      console.warn('Payment tracking columns not found in database. Updating without them. Please run add_payment_tracking.sql migration.');
      
      // Remove payment tracking fields and retry
      delete mappedData.paid_by;
      delete mappedData.paid_by_role;
      delete mappedData.paid_by_email;
      delete mappedData.paid_by_id;
      delete mappedData.payment_recorded_at;
  ...
  try {
    ...
    if (isOnline && shouldUseSupabase()) {
      ...
      const mappedFees = feesData.map(feeData => {
        ...
        // Payment tracking fields
        if (feeData.paidBy || feeData.paid_by) {
          minimalUpdate.paid_by = feeData.paidBy || feeData.paid_by;
        }
        if (feeData.paidByRole || feeData.paid_by_role) {
          minimalUpdate.paid_by_role = feeData.paidByRole || feeData.paid_by_role;
        }
        if (feeData.paidByEmail || feeData.paid_by_email) {
          minimalUpdate.paid_by_email = feeData.paidByEmail || feeData.paid_by_email;
        }
        if (feeData.paidById || feeData.paid_by_id) {
          minimalUpdate.paid_by_id = feeData.paidById || feeData.paid_by_id;
        }
        if (feeData.paymentRecordedAt || feeData.payment_recorded_at) {
          minimalUpdate.payment_recorded_at = feeData.paymentRecordedAt || feeData.payment_recorded_at;
        }
  ...
  try {
    ...
    if (isOnline && shouldUseSupabase()) {
      ...
      const mappedInstallments = installmentsData.map(inst => ({
        ...
        // Payment tracking fields
        paid_by: inst.paidBy || inst.paid_by,
        paid_by_role: inst.paidByRole || inst.paid_by_role,
        paid_by_email: inst.paidByEmail || inst.paid_by_email,
        paid_by_id: inst.paidById || inst.paid_by_id,
        payment_recorded_at: inst.paymentRecordedAt || inst.payment_recorded_at,
        updated_at: new Date().toISOString()
      }));
  ...
  if (response.success && response.data) {
    response.data = response.data.map((installment: any) => {
      const mappedInstallment = {
        ...
        // Payment tracking fields
        paidBy: installment.paid_by,
        paidByRole: installment.paid_by_role,
        paidByEmail: installment.paid_by_email,
        paidById: installment.paid_by_id,
        paymentRecordedAt: installment.payment_recorded_at,
        paid_by: installment.paid_by,
        paid_by_role: installment.paid_by_role,
        paid_by_email: installment.paid_by_email,
        paid_by_id: installment.paid_by_id,
        payment_recorded_at: installment.payment_recorded_at
      };
...
export const updateInstallment = async (id: string, installmentData: any): Promise<ApiResponse> => {
  ...
  const mappedData: Record<string, any> = {
    ...
    ...Object.fromEntries(
      Object.entries(installmentData).filter(([key]) => 
        !['studentId', 'studentName', 'schoolId', 'feeId', 'feeType', 'dueDate', 'paidDate', 'paidAmount', 
          'installmentCount', 'installmentNumber', 'installmentMonth', 'paymentMethod', 'paymentNote', 
          'receiptNumber', 'checkNumber', 'checkDate', 'bankNameArabic', 'bankNameEnglish', 'createdAt', 'updatedAt', 'paymentDate',
          'paidBy', 'paidByRole', 'paidByEmail', 'paidById', 'paymentRecordedAt'].includes(key)
      )
    ),
    // Add calculated fields
    status, // Use calculated status
  ...
  // Payment tracking fields - only include if they have values
  // This prevents errors if the columns don't exist in the database yet
  if (installmentData.paidBy || installmentData.paid_by) {
    mappedData.paid_by = installmentData.paidBy || installmentData.paid_by;
  }
  if (installmentData.paidByRole || installmentData.paid_by_role) {
    mappedData.paid_by_role = installmentData.paidByRole || installmentData.paid_by_role;
  }
  if (installmentData.paidByEmail || installmentData.paid_by_email) {
    mappedData.paid_by_email = installmentData.paidByEmail || installmentData.paid_by_email;
  }
  if (installmentData.paidById || installmentData.paid_by_id) {
    mappedData.paid_by_id = installmentData.paidById || installmentData.paid_by_id;
  }
  if (installmentData.paymentRecordedAt || installmentData.payment_recorded_at) {
    mappedData.payment_recorded_at = installmentData.paymentRecordedAt || installmentData.payment_recorded_at;
  }
  ...
  // Try to update with payment tracking fields first
  try {
    return await update(STORAGE_KEYS.INSTALLMENTS, id, mappedData);
  } catch (error: any) {
    // If we get a PGRST204 error about missing columns, retry without payment tracking fields
    if (error?.code === 'PGRST204' || error?.message?.includes('paidBy') || error?.message?.includes('paid_by')) {
      console.warn('Payment tracking columns not found in database. Updating without them. Please run add_payment_tracking.sql migration.');
