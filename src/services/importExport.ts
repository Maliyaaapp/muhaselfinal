...
const mapInstallmentRow = (i: any) => ({
  ...
  installment_interval: Number(i.installmentInterval || 1), // Interval in months between installments
...
interface ImportedInstallment {
  ...
  installmentInterval?: number;  // Interval in months between installments (1=monthly, 2=every 2 months, etc.)
  note?: string;
  isPaid?: boolean;
  paidDate?: string;
  paidAmount?: number;
...
/**
 * Parses a date string in various formats (DD/MM/YYYY, DD-MM-YYYY, YYYY-MM-DD, etc.)
 * and returns a Date object. Handles Arabic date formats commonly used in CSV imports.
 */
export const parseDateString = (dateStr: string | undefined): Date => {
  ...
  // DD/MM/YYYY format (common in Arabic regions)
...
export const parseCSV = (text: string): Array<any> => {
  ...
  const headerMap: Record<string, string> = {
    ...
    'فترة الأقساط': 'installmentInterval',
  ...
  data.forEach(row => {
    ...
    if (mappedRow.name && mappedRow.studentId) {
      ...
      if (hasCombinedFees) {
        ...
        const numberOfInstallments = mappedRow.numberOfInstallments ? parseInt(mappedRow.numberOfInstallments, 10) : 0;
        const installmentStartDate = mappedRow.installmentStartDate || getFirstDayOfMonth(new Date().toISOString().split('T')[0]);
        if (numberOfInstallments > 0) {
    ...
    // Check if this is an installment row
      ...
      if (!installmentMonth && mappedRow.dueDate) {
        try {
          // Use parseDateString to properly handle DD/MM/YYYY format from CSV
          const dueDate = parseDateString(mappedRow.dueDate);
          const monthNames = [
  ...
  try {
    ...
    for (const student of students) {
      ...
      for (const fee of fees) {
        ...
        const existingStudent = existingStudents.find((s: any) => s.studentId === fee.studentId);
      ...
      for (const installment of installments) {
        ...
        const existingStudent = existingStudents.find((s: any) => s.studentId === installment.studentId);
          ...
          if (!feeId) {
            ...
            if (matchingFee) {
              ...
              if (installment.feeType === 'transportation_and_tuition') {
                ...
                const tuitionFee = existingFees.find((f: any) => f.feeType === 'tuition');
                const transportationFee = existingFees.find((f: any) => f.feeType === 'transportation');
...
export const saveImportedDataWithSync = async (
  ...
  try {
    ...
    if (allFeesResponse2.success) {
      ...
      const feesToUpdate = allFeesResponse2.data.filter((fee: any) => 
      ...
      for (const fee of feesToUpdate) {
        try {
          ...
          const totalPaid = feeInstallments.reduce((sum: number, inst: any) => {
...
export const exportFeesToCSV = async (schoolId: string): Promise<string> => {
  ...
  const rows = await Promise.all(fees.map(async (fee: any, _index: number) => {
  ...
  const totalDiscount = fees.reduce((sum: number, fee: any) => sum + fee.discount, 0);
  const totalPaid = fees.reduce((sum: number, fee: any) => sum + fee.paid, 0);
...
export const exportInstallmentsToCSV = async (schoolId: string): Promise<string> => {
  ...
  const installments: InstallmentWithMonth[] = rawInstallments.map((installment: any) => {
    const student = students.find((s: any) => s.id === installment.studentId) || {
      name: 'طالب غير معروف',
      grade: 'غير محدد',
  ...
  const rows = installments.map((installment: any) => {
